  ...
  return (
    <DropdownMenu.Root onOpenChange={(open) => open && setTimeout(animateIn, 10)}>
      <DropdownMenu.Trigger asChild>
        <button suppressHydrationWarning={true}  className="relative cursor-pointer p-2 rounded-full text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors outline-none focus:ring-2 focus:ring-indigo-500/20">
          <Bell size={20} />
        ...
        >
          ...
          <div className="space-y-1">
            {notifications.map((item) => (
              ...
              >
                <div className={clsx("mt-0.5 p-1.5 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700")}>
                  {getIcon(item.type)}
                </div>
